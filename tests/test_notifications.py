"""
Tests para Phase 4: Mis Reclamos y Notificaciones
...
class TestNotifications(BaseTestCase):
    ...
    def test_get_user_supported_claims(self):
        ...
        # user2 se adhiere
        ClaimService.add_supporter(claim_id=claim.id, user_id=self.user2_id)

        # Obtener reclamos adheridos por user2
        supported_claims = ClaimService.get_user_supported_claims(self.user2_id)
    ...
    def test_update_claim_status(self):
        ...
        success, error = ClaimService.update_claim_status(
            claim_id=claim.id,
    ...
    def test_update_status_creates_notification(self):
        ...
        notifications_before = NotificationService.get_pending_notifications(self.user1_id)
        ...
        notifications_after = NotificationService.get_pending_notifications(self.user1_id)
        ...
        self.assertEqual(notification.claim_status_history.new_status, ClaimStatus.IN_PROGRESS)
    ...
    def test_supporter_receives_notification(self):
        ...
        # Admin cambia el estado
        ClaimService.update_claim_status(
            claim_id=claim.id, new_status=ClaimStatus.RESOLVED, admin_user_id=self.admin_id
        )

        # Verificar que ambos usuarios reciben notificación independiente
        user1_notifications = NotificationService.get_pending_notifications(self.user1_id)
        user2_notifications = NotificationService.get_pending_notifications(self.user2_id)
    ...
    def test_get_unread_count(self):
        ...
        count = NotificationService.get_unread_count(self.user1_id)
    ...
    def test_mark_notification_as_read(self):
        ...
        notifications = NotificationService.get_pending_notifications(self.user1_id)
    ...
    def test_mark_all_notifications_as_read(self):
        ...
        count_before = NotificationService.get_unread_count(self.user1_id)
        self.assertEqual(count_before, 2)

        # Marcar todas como leídas
        marked = NotificationService.mark_all_as_read(self.user1_id)
        self.assertEqual(marked, 2)

        # Verificar que no quedan notificaciones
        count_after = NotificationService.get_unread_count(self.user1_id)
    ...
    def test_no_notification_if_status_unchanged(self):
        ...
        success, error = ClaimService.update_claim_status(
            claim_id=claim.id, new_status=ClaimStatus.PENDING, admin_user_id=self.admin_id
        )
    ...
    def test_notifications_are_independent_per_user(self):
        ...
        self.assertEqual(NotificationService.get_unread_count(self.user1_id), 1)
        self.assertEqual(NotificationService.get_unread_count(self.user2_id), 1)
        ...
        self.assertEqual(NotificationService.get_unread_count(self.user1_id), 0)
        self.assertEqual(NotificationService.get_unread_count(self.user2_id), 1)
        ...
        self.assertEqual(NotificationService.get_unread_count(self.user1_id), 0)
        self.assertEqual(NotificationService.get_unread_count(self.user2_id), 0)
